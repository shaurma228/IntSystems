...
# На полном California Housing 30 шумовых признаков почти не ломают модель,
# потому что объектов слишком много. Для учебной демонстрации переобучения
# берем небольшой фрагмент train после честного train/test split.
DEMO_TRAIN_SIZE = 45
...
def make_demo_train_subset(
    ...
    selected_indices = rng.choice(
        ...
        size=min(train_size, len(X_train)),
        replace=False,
    )
...
def main() -> None:
    ...
    X_train, y_train = make_demo_train_subset(X_train_full, y_train_full)

    print(f"Train после обычного split: {X_train_full.shape}")
    print(f"Test после обычного split:  {X_test.shape}")
    print(
        f"Учебный train-поднабор для демонстрации переобучения: {X_train.shape}. "
        "Test не уменьшается."
    )
    ...
    if broken_test_r2 < baseline_test_r2:
        ...
        print(
            "R2_test не стал хуже. В этом запуске шум оказался не настолько вредным; "
            "для более сильной поломки можно уменьшить DEMO_TRAIN_SIZE."
        )
